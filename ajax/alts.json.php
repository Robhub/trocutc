...
$requete = "SELECT A.*,COUNT(B.login) tdtps, COUNT(C.login) cours FROM cours A LEFT JOIN $B LEFT JOIN $C WHERE A.login NOT IN ($nospam) AND A.uv=:uv AND A.type=:type GROUP BY A.login,A.uv,A.jour,A.debut ORDER BY A.login";
